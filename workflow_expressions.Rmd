...
files would be `read.csv`). This file should be stored within the users working
...
We can see that the proportion of missing values in our data is only `r format(nNA(cp_qf[["psms_filtered"]])$nNA$pNA, digits = 1)`, corresponding to `r nNA(cp_qf[["psms_filtered"]])$nNA$nNA` NA values. This low proportion is due to a
...
post-acquisition data normalization is to minimize the biases that arises due to
...
wishing to explore and compare normalization methods, the [`NormalyzerDE`](https://www.bioconductor.org/packages/release/bioc/html/NormalyzerDE.html)
...
method of normalization that we wish to apply i.e. `method = "center.median"` and 
name the new data level e.g., `name = "log_norm_proteins"`. Of note, for users who
wish to apply VSN normalization the raw protein data must be passed (prior to any
...
normalization (left), log2 transformed (middle) and post-normalization (right). 
...
the protein data prior to normalization.
...
`r format(nNA(sn_qf[["peptides_filtered"]])$nNA$pNA, digits = 1)` of the data.
