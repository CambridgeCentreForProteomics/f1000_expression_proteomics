...
`.txt` file. Such files are the outputs of most major third-party search
...
is provided in the [appendix](https://github.com/CambridgeCentreForProteomics/f1000_expression_proteomics).
...
with the dataset identifier PXD041794, Zenodo at [http://doi.org/10.5281/zenodo.11196770](http://doi.org/10.5281/zenodo.11196770)
...
[`QFeatures`, Bioconductor package](https://www.bioconductor.org/packages/release/bioc/html/QFeatures.html) [@QFeat]. Prior to utilizing the `QFeatures` infrastructure, it is first necessary to
understand the structure of a [`SummarizedExperiment`](https://bioconductor.org/packages/release/bioc/html/SummarizedExperiment.html) [@SumExp] object as `QFeatures` objects are based on the `SummarizedExperiment`
...
the use of `QFeatures` can be found in [@QFeat]. For visualisation of the data,
...
the [appendix](https://github.com/CambridgeCentreForProteomics/f1000_expression_proteomics).
...
using `QFeatures`. First, data is imported from our PSM-level `.txt` file into a 
`data.frame` using the base R `read.delim` function (the equivalent for `.csv` 
files would be `read.csv`). This file should be stored within the user's working
directory. Next, we use the `names` function to inspect the column names and 
...
`data.frame` to the `readQFeatures` function and specify where our quantitative
columns are. The latter is done by passing either a numeric or character vector
to an argument called `quantCols` (`ecols` in previous package versions). 
...
object. Whilst previous `QFeatures` vignettes used the "Sequence" or 
...
cases, multiple rows would have the same name forcing the `readQFeatures` function 
...
variable, we can look at the `assay` slot.
...
Our `assay` contains data of type `double`, a sub-type of numeric data in R. If
users find that their quantitative data is of type `character`, it is necessary
...
is added to the `colData` slot. It is important for users check that the `colData` 
...
and informative. This is done by editing the `colnames`. These steps may not 
...
`r format(original_prots, scientific = FALSE)` master proteins or protein groups.
Finally, we confirm that the identification search was carried out as expected. 
For this, we print summaries of the key search parameters using the `table` function 
for discrete parameters and `summary` for those which are continuous. This is also
...
into a `data.frame` to format into a `SummarizedExperiment` object. This is done
using the `readSummarizedExperiment` function and the same arguments as those
in `readQFeatures`. The reason for this is that we only need the PSM-level data,
so do not need to generate a `QFeatures` object with the function of holding
multiple `SummarizedExperiments` across different levels. 
...
argument is not used, `filterFeatures` will remove features from all levels 
(`SummarizedExperiments`) within a `QFeatures` object. 
...
FDR, we have to import the protein-level `.txt` file from our database search.
...
The FDR information is contained in a column called `Protein.FDR.Confidence.Combined`.
We can get an overview of the data in this column by using the `table` function.
...
We see proteins that are annotated as high confidence = FDR < 1%, medium 
confidence = FDR < 5% and low confidence = FDR > 5%.

In order to use the `filterFeatures` function to remove PSMs corresponding to 
proteins which do not pass our FDR threshold (1%), we first need to add this
annotation to the `rowData` of our `"psms_filtered"` experiment. To do this, we 
...
## Extract protein accessions and confidence from search output file
...
Finally, we add the annotations from our `"Protein.FDR.Confidence.Combined"`
column to the `rowData` of our `"psms_filtered"` experiment.

```{r}
rowData(cp_qf[["psms_filtered"]])$Protein.Confidence <- 
  protein_fdr$Protein.FDR.Confidence.Combined
```

We can now use `filterFeatures` to remove PSMs corresponding to low or medium
...
peptides and protein groups have been removed throughout the in-depth data-specific
filtering.

```{r tmt_filtering_summary_3}
## Summarise the effect of data-specific filtering
...
of the `zeroIsNA` or `infIsNA` functions within the `QFeatures` infrastructure.
...
We can see that the proportion of missing values in our data is only `r format(nNA(cp_qf[["psms_filtered"]])$nNA$pNA, digits = 1, scientific = FALSE)`, corresponding to `r nNA(cp_qf[["psms_filtered"]])$nNA$nNA` NA values. This low proportion is due to a
...
## Plot histogram to visualise the distribution of NAs
...
explicit `QFeatures` links. This means grouping PSMs by "Sequence".
...
`"Annotated.Sequence"` to preserve this information.
...
wishing to explore and compare normalisation methods, the [`NormalyzerDE`](https://www.bioconductor.org/packages/release/bioc/html/NormalyzerDE.html)
package within Bioconductor can be used [@Willforss2018]. 

Here, we will demonstrate the use of a center median normalisation approach using 
the `normalize` function within `QFeatures`. To do this, we pass the log transformed
protein-level data to the `normalize` function in `QFeatures`. We specify the
method of normalisation that we wish to apply i.e. `method = "center.median"` and 
name the new data level e.g., `name = "log_norm_proteins"`. Of note, for users who
...
`method = "vsn"`. All other methods require users to explicitly perform log 
transformation on their data before use. More details can be found in the
`QFeatures` documentation, please type `help("normalize,QFeatures-method")`.

```{r tmt_normalisation_1}
## normalise the log transformed peptide data
...
```{r tmt_normalisation_2, out.height = '40%', fig.align = 'center'}
## Evaluate the effect of data normalisation
pre_norm <- cp_qf[["log_proteins"]] %>%
  ...
  geom_boxplot() +
  labs(x = "Sample", y = "log2(abundance)", title = "Pre-normalisation") +
  theme_bw()
...
post_norm <- cp_qf[["log_norm_proteins"]] %>%
  ...
  ggplot(aes(x = colname, y = value, fill = Condition)) +
  geom_boxplot() +
  labs(x = "Sample", y = "log2(abundance)", title = "Post-normalisation") +
  theme_bw()

pre_norm + theme(legend.position = "none") + post_norm
```
...
## visualise the process of log transformation and normalisation
par(mfrow = c(1, 3))
...
peptides and `r nrow(Q01581[["psms_filtered"]])` PSMs supporting its identification 
...
`".n"` from the protein-level to explore peptide support per protein.
...
users should export a peptide-level `.txt` file from the results of their 
...
data files, such as the `evidence.txt` file generated by MaxQuant. 

### Importing data into a QFeatures object
We locate the PeptideGroups `.txt` file and import this into a `data.frame`
...
In the code chunk below, we again use the `readQFeatures` function to create a
`QFeatures` object. We find the abundance data is located in columns 21 to 26
and thus pass this to `quantCols`. After import we annotate the `colData`.
...
`r original_prots` master proteins or protein groups. Finally, we take a look at
...
7. Control of FDR to 1% protein-level FDR (or 5% for more exploratory analyses)
...
at the highest possible level. See above for more details.
...
We can now use `filterFeatures` to remove peptides corresponding to low or medium
confidence master proteins.
...
`r format(nNA(sn_qf[["peptides_filtered"]])$nNA$pNA, digits = 2)` of the data.
...
## Plot histogram to visualise sample-specific distribution of NAs
...
#### Expectations of missing values
...
## visualise the impact of imputation
par(mfrow = c(1, 2))
...
The final dataset is comprised of `r nrow(sn_qf[["log_norm_proteins"]])` protein
groups. We will save the protein-level `SummarizedExperiment` file as well as
exporting the final `QFeatures` object.
...
pipelines both label-based and LFQ experiments [@Zhu2020]. For LFQ experiments, 
[`proDA`](https://bioconductor.org/packages/release/bioc/html/proDA.html),
[`prolfqua`](https://rdrr.io/github/wolski/prolfqua/) and 
[`MSqRob2`](https://www.bioconductor.org/packages/release/bioc/html/msqrob2.html) 
can be utilised, among others [@Goeminne2020; @Wolski2023]. Here, we will use the
...
limma_results$direction <- 
  ifelse(limma_results$logFC > 0, "up", "down") %>%
  as.factor()

## Add significance thresholds
limma_results$significance <- 
  ifelse(limma_results$adj.P.Val < 0.01, "sig", "not.sig") %>%
  as.factor()
...
labels and wished to include a logFC threshold, we could have included the `lfc` argument
...
fitted_lm %>%
  decideTests(adjust.method = "BH", p.value = 0.01) %>%
  summary()
```
...
sig_proteins <- subset(limma_results, adj.P.Val <= 0.01)
...
```{r volcano_plot, out.height = '40%', fig.align = 'center', warning = FALSE}
...
```{r MA_plot,  out.height = '40%', fig.align = 'center', warning = FALSE}
## Generate MA plot
limma_results %>%
...
write.csv(limma_results, file = "all_limma_results.csv")

## Save subsets of upregulated and downregulated proteins
write.csv(sig_up, file = "upregulated_results.csv")
write.csv(sig_down, file = "downregulated_results.csv"))
...
[http://doi.org/10.5281/zenodo.11196770](http://doi.org/10.5281/zenodo.11196770),
